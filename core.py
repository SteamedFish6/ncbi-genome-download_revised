...
def argument_parser(version=None):
    ...
    parser.add_argument('-D', '--max-download', dest='max_download', type=int,
                        default=NgdConfig.get_default('max_download'),
                        help='Set a maximum number of download candidates (default: %(default)s).') # add by myself
...
def select_candidates(config):
    ...
    if len(download_candidates) > config.max_download:
        download_candidates = download_candidates[:config.max_download] # add by myself
